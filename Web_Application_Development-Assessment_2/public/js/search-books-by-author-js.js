...
const makeTable = function () {
    ...
    var txt = "";
    txt += "<table><tr><th>ID</th><th>Title</th><th>Author</th><th>ISBN</th><th>Loan Status</th><th>See Loan Details</th><th>Delete book?</th></tr>"
    for (x in response) {
        let book_author = response[x].name;
        for (y in response[x].Books) {
            txt += "<tr><td>" + response[x].Books[y].id + "</td><td>" + response[x].Books[y].title + "</td><td>" + book_author + "</td><td>" + response[x].Books[y].isbn + "</td><td>" + getLoans(response[x].Books[y].id) + "</td><td><input type='button' onclick='makeLoanRequest(" + response[x].Books[y].id + ")' value='Details'></td><td><input type='button' onclick='deleteButton(" + response[x].Books[y].id + ")' value='Delete'></td></tr>";
        };    
    };
    txt += "</table>";
...
//SEARCH FOR A BOOK BY THE AUTHOR
const makeAPIQuery = function(search_term) {
    ...
    let queryURL = rootURL + "?type=authorbooks&" + encodeParameters(params);
...
// SEARCH BUTTON
let submitButton = document.getElementById("search_by_author_button");
