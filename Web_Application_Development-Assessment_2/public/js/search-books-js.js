...
// GET A LIST OF THE IDS OF THE BOOKS THAT ARE ON LOAN
...
const makeTable = function () {
    ...
    var txt = "";
    txt += "<table><tr><th>ID</th><th>Title</th><th>ISBN</th><th>Loan Status</th><th>See Loan Details</th><th>Delete book?</th></tr>"
    for (x in response) {
        txt += "<tr><td>" + response[x].id + "</td><td>" + response[x].title + "</td><td>" + response[x].isbn + "</td><td>" + getLoans(response[x].id) + "</td><td><input type='button' onclick='makeLoanRequest(" + response[x].id + ")' value='Details'></td><td><input type='button' onclick='deleteButton(" + response[x].id + ")' value='Delete'></td></tr>";
    };
    txt += "</table>";
...
// SEARCH BUTTON
let submitButton = document.getElementById("search_by_title_button");
