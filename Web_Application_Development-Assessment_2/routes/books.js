...
router.get("/", function(req, res) {
    if (req.query.allEntities == "true") {
        db.Book.findAll({ include: [db.Author, db.Publisher] }).then(function(books) {
            ret.json(books, res);
        });
    } else {
...
router.post("/:bookID/publishers", function(req, res) {
    db.Book.findByPk(req.params.bookID, { include: [db.Publisher] }).then(function(book) {
        if (book) {
            db.Publisher.findOrCreate({ where: { name: req.body.name } }).spread(function(
                publisher,
                created
            ) {
