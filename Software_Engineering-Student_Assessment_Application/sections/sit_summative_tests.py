...
class SummativeTestMenu(Frame):
	...
	def getMenu(self):
		...
		lblTestTitle = Label(self, text='Summative Test Menu', font=('Courier New', 25, 'bold'), fg="firebrick3")
		lblTestTitle.grid(row=0, column=0, columnspan=5, padx=10, pady=10, sticky=N)
		
		lblTestTitle = Label(self, text='Here is a list of available tests. \n Please select the test you would like to attempt and ensure you are fully prepared before starting.', font=('verdana', 12, 'normal'))
		lblTestTitle.grid(row=2, column=0, columnspan=5, padx=70, pady=(0,30), sticky=N)

		# Make a list of tests in the test directory
		self.testList = os.listdir("tests/summative/CMT001/")	
	...
	def openTest(self):
		...
		try:
			...
			if os.path.isfile("./results/summative/" + selectedTest):
				pass
			else:
				with open ("./results/summative/" + selectedTest, mode="a") as results_csv:
...
class Test(Frame):
	...
	#show the test name, number of attempts and button to return to the summative test menu
	def getTestTitle(self):
		...
		resultsInfo = pd.read_csv("./results/summative/" + selectedTest)
		...
		lblTestTitle = Label(self, text=testName, font=('verdana', 30, 'bold'))
		lblTestTitle.grid(row=0, column=0, columnspan=5, padx=10, pady=(10,0), sticky=NW)

		if self.attempts == 0:
			self.lblAttempts = Label(self, text="You have 1 attempt. Please ensure you take care before submitting. ", font=('verdana', 16, 'normal'), justify=LEFT)
			self.lblAttempts.grid(row=1, column=0, columnspan=10, padx=10, sticky=W)

		else:
			self.lblAttempts = Label(self, text="You have 1 attempt. Please ensure you take care before submitting. ", font=('verdana', 16, 'bold'), justify=LEFT)
		...
		# Return to main menu
		buttonMainMenu = Button(self, text='Exit without Submitting', font=('verdana', 12, 'normal'), width=20, height=2)
	...
	def getTestQuestions(self):
		...
		with open("tests/summative/CMT001/" + selectedTest) as test_file:
	...
	# Check the selected answers and give a score
	def storeResponse(self):

		# If the submit button is pressed when the limit is reached show a pop up message
		if self.attempts >= 1:
			tkinter.messagebox.askokcancel("Warning", "Are you sure you would like to submit this test? Once you click OK, this will be final and your results will be submitted for assessment.")
